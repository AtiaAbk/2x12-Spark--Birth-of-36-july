...
public class DhakaCampusWorld implements Disposable {
    ...
    private void buildCampus(TextureFactory textures) {
        ...
        Material curzonDomeMat = new Material(ColorAttribute.createDiffuse(new Color(0.90f, 0.88f, 0.82f, 1f)));
        ...
        // 2. Grand Herringbone Brick Avenue (Walkway to Curzon Hall) - Tiled in 7.2m square segments
        ...
        for (int i = -3; i <= 3; i++) {
            ...
            arch.transform.setTranslation(i * 4.4f, 3.8f, -23.8f);
            instances.add(arch);
        }
        ...
        Model domeBase = mb.createCylinder(8.2f, 2.5f, 8.2f, 20, curzonTrimMat, attr);
        Model domeSphere = mb.createSphere(7.8f, 5.8f, 7.8f, 20, 20, curzonDomeMat, attr);
        models.add(domeBase);
        models.add(domeSphere);

        ModelInstance domeBaseInst = new ModelInstance(domeBase);
        domeBaseInst.transform.setTranslation(0f, 16.8f, -22.5f);
        instances.add(domeBaseInst);

        ModelInstance domeInst = new ModelInstance(domeSphere);
        domeInst.transform.setTranslation(0f, 19.8f, -22.5f);
        instances.add(domeInst);

        // Corner Chhatris (4 authentic Indo-Saracenic domed kiosks)
        Model chhatriDome = mb.createSphere(3.2f, 2.5f, 3.2f, 16, 14, curzonDomeMat, attr);
